...
import { ensureComparisonValue, getConditionType, getConfigJsonSalt, getSettingType, getTimestampAsDate, hasPercentageOptions, inferSettingType, InvalidConfigModelError, isCompatibleValue, nameOfSettingType, throwInvalidConfigModelError, unwrapValue } from "./ProjectConfig";
...
type PickWithType<T, TPropMap extends { [K in keyof T]?: unknown }> = {
...
export function evaluationDetailsFromDefaultValue<T extends SettingValue>(key: string, defaultValue: T,
  ...
  const evaluationDetails: EvaluationDetails<SettingTypeOf<T>> & { _errorMessage: Message } = {
    key,
