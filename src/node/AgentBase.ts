...
// Based on: https://github.com/TooTallNate/proxy-agents/blob/agent-base%407.1.4/packages/agent-base/src/index.ts

// NOTE: We need to augment the http.Agent type as some necessary properties and methods are not defined in `@types/node`.
declare module 'http' {
  interface Agent {
...
export abstract class Agent extends http.Agent {
  ...
  private incrementSockets(name: string) {
    ...
    if (!this.sockets[name]) {
      (this.sockets as Mutable<typeof this.sockets>)[name] = [];
    }
  ...
  private decrementSockets(name: string, socket: null | net.Socket) {
    ...
    if (index !== -1) {
      ...
      this.totalSocketCount--;
      if (sockets.length === 0) {
        delete (this.sockets as Mutable<typeof this.sockets>)[name];
      }
    }
  }
